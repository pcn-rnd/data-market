  ...
  export default {
    ...
    components: {Layout, PageHeader},
      ...
      methods: {
          ...
          getDs() {
              axios.get(`/ds/${this.$route.query.setId}?m=true`)
                  .then(res => {
                        if(res.status === 200) {
                            this.dataset = res.data.body;
                            this.urlsInfo = JSON.parse(this.dataset.urlsInfo);
          ...
          getFList() {
              ...
              axios.post('/file/f-list', this.dataset.path)
              .then(res => {
                                  ...
                                  <tr v-for="(info, index) in urlsInfo" :key="info.id" >
                                      <td>
                                          <select :id="`types_${index}`" v-model="info.datatype" class="form-control col-md-12" @change="selectType(index, $event)">
                                              <option v-for="(item, idx) in types" :key="idx" :id="`typ_opt_${index}_${idx}`">{{item}}</option>
                                          </select>
                          ...
                          <div class="form-group row mb-4" v-if="fileList.length != 0">
                              ...
                              <div class="col-lg-10 pl-4">
                                  <p class="text-muted" v-for="(file, idx) in fileList" :key="idx">
                                      <i class="bx bx-paperclip font-size-16 align-middle text-primary mr-1"></i> {{file}}
